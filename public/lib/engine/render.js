...
import { drawing_functions } from './visual/stave_symbols.js';

export function ABCRenderer(ractive) {
    ...
    var renderLine = function (line, drawnLineIndex, lineIndex) {        
        ...
        var leadInGroup = s("g");
        lineGroup.children.push(drawing_functions.stave(), leadInGroup);

        var clef = drawing_functions.treble_clef();
        var keySig = drawing_functions.keysig(settings.key, clef.width, line.id, ractive.get('currentTranspositionValue'));
        ...
        if (drawnLineIndex === 0) {
            var timeSig = drawing_functions.timesig(settings.measure.top, settings.measure.bottom, clef.width + keySig.width);
        ...
        for (let i = 0; i < line.symbols.length; i++) {
            ...
            symbolsGroup.children.push(drawing_functions[line.symbols[i].type](line.symbols[i], xPos, noteAreaWidth));
            xPos += (line.symbols[i].fixedWidth + springMod * line.symbols[i].springConstant);
        }

        for(let i = 0; i < line.endings.length; i++) {
            symbolsGroup.children.push(drawing_functions.varientEndings(line.endings[i], noteAreaWidth, false));
        }

        for(let i = 0; i < line.tuplets.length; i++) {
            symbolsGroup.children.push(drawing_functions.tuplets(line.tuplets[i], noteAreaWidth));
        }

        if(nextLineStartsWithEnding) {
            symbolsGroup.children.push(drawing_functions.varientEndings({
                name: "",
                start: {
                    xp: 0
                },
